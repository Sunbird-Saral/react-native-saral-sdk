...
def summarize_performance(step, g_model, latent_dim, n_samples=100):
	...
	model_file_name =config.MODEL_SAVE_PATH+"_"+str(step+1)+"_.h5"
...
def train(g_model, c_model, gan_model, dataset, latent_dim, n_epochs=1000, n_batch=64, n_critic=5):
	...
	for i in range(n_steps):
		...
		if (i+1) % (bat_per_epo*config.SAVE_EPOCHS) == 0:
...
if config.FINE_TUNE_MODEL_PATH is not None:
	generator = load_model(config.FINE_TUNE_MODEL_PATH)
else:
	generator = define_generator(config.latent_dim)
# create the gan
...
# train model
train(generator, critic, gan_model, dataset, config.latent_dim)